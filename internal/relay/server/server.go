...
import (
	...
	"github.com/KamorionLabs/aws-smtp-relay/internal/relay/factory"
...
// NewCramMD5Server implements the CRAM-MD5 authentication mechanism, as
// described in RFC 2195.
// The returned Server uses the given username and secret to authenticate to the
...
func StartSMTPServer(inCfg config.Config, myLog *log.Logger, clientFn func(srv *smtp.Server, lsr net.Listener), sopts ...ServerOpts) error {
	...
	// Create listener
	lsr, err := net.Listen("tcp", srv.Addr)
	if err != nil {
